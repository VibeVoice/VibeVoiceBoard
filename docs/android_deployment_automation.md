...
mkdir -p "$DEST_DIR"
cp app/build/outputs/apk/debug/VibeVoiceKeyboard_<version>-<versionCode>-debug.apk "$DEST_DIR/"
```
...
if adb devices | grep -q "$DEVICE_IP.*device"; then
  ...
  adb -s "$DEVICE_IP" install -r app/build/outputs/apk/debug/VibeVoiceKeyboard_<version>-<versionCode>-debug.apk
else
